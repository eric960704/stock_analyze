...
# 布林通道
def bbandCal(price, time, nup, ndn):
    H_line, M_line, L_line = tb.BBANDS(price, timeperiod=time, nbdevup=nup, nbdevdn=ndn, matype=0)
...
def best_BBands(stock_price,BB_ma_min,BB_ma_max,max_op,min_op):
    final_sumpal = -10000  # 假設損益初始值
    BH = stock_price.Open[-1] - stock_price.Open[0]  # 股票漲幅
    better_num = {"代碼": 0, "最佳損益": 0.0, "股票漲幅": 0.0, "最佳布林均線": 0, "交易次數": 0,"獲利次數": 0, "虧損次數": 0,
                  "平均每次交易損益": 0.0, "平均每次交易勝率": 0.0, "單次最大獲利": 0.0,"單次最大虧損": 0.0}

    for j in range(BB_ma_min,BB_ma_max+1):
        stock_price['upperBB'], stock_price['maBB'], stock_price['lowerBB'] = bbandCal(stock_price.Close, j, 2, 2)
        sign = []  # 買賣訊號
        retain = []  # 留倉
        sumpal = []  # 策略累計損益
        pal = []  # 當前損益
        ...
        for i in range(1,len(stock_price)):
            """交易訊號"""
            if stock_price.Close[i-1] < stock_price['upperBB'].values[i-1] and stock_price.Close[i] > stock_price['upperBB'].values[i]:
                sign.append(1)
            elif stock_price.Close[i-1] > stock_price['lowerBB'].values[i-1] and stock_price.Close[i] < stock_price['lowerBB'].values[i]:
                sign.append(-1)
            else:
                sign.append(0)
            """交易訊號"""
...
"""輸入代碼與日期回測"""
def simple_back_boiling(id,ma_min,ma_max,StartDate,EndDate):#單一股票取得結果
    ...
    cmp = best_BBands(stock_price,ma_min,ma_max, 1, -1)

    return cmp
"""輸入代碼與日期回測"""

"""檢查訊號"""
def CheckBB(stock_id,BB_MA) :
    stock_price = stock.loc[(stock.Id == stock_id)]
    stock_price = stock_price.set_index('Date')
    stock_price['upperBB'], stock_price['maBB'], stock_price['lowerBB'] = bbandCal(stock_price.loc[:,"Close"].values, BB_MA, 2, 2)

    if stock_price.Close.iloc[-2] < stock_price.upperBB.iloc[-2] and stock_price.Close.iloc[-1] > \
            stock_price.upperBB.iloc[-1]:
        return 1
    elif stock_price.Close.iloc[-2] > stock_price.lowerBB.iloc[-2] and stock_price.Close.iloc[-1] < \
            stock_price.lowerBB.iloc[-1]:
        return -1
    else:
        return 0
"""檢查訊號"""